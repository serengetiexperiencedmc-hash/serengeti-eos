...
} from "@/lib/mappings-api";

function statusBadge(status: RegulationControlMapping["status"]) {
...
export default function MappingsPage() {
  ...
  const [items, setItems] = useState<RegulationControlMapping[]>([]);
  const [obligations, setObligations] = useState<ComplianceObligation[]>([]);
  const [controls, setControls] = useState<GrcControl[]>([]);
  const [health, setHealth] = useState<{ mappings: number; openMappings: number } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [list, h] = await Promise.all([listMappings(token), getMappingsHealth(token)]);
      setItems(list.items);
      setHealth({ mappings: h.mappings, openMappings: h.openMappings });
      try {
        const [obl, ctl] = await Promise.all([listObligations(token), listControls(token)]);
      ...
      setItems([]);
      setError(err instanceof EosApiError ? err.message : "Failed to load mappings");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((row) => {
      ...
      if (!q) return true;
      return `${row.mappingCode} ${row.title}`.toLowerCase().includes(q);
    });
  }, [items, query, statusFilter]);

  const selected = items.find((row) => row.id === selectedId) ?? null;

  if (ready && !token) return <p className="text-sm text-muted">Sign in to view mappings.</p>;

  return (
    <>
      <PageHeader
        eyebrow="G5 · Compliance"
        title="Mappings"
        subtitle="Regulation-to-control mapping register only · not a test engine or legal interpretation"
        actions={
          <Btn variant="secondary" href="/commercial/control-tests">
            ← Control tests
          </Btn>
        }
      />
      ...
      {health && (
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Card title="Mappings">
            <div className="font-display text-2xl font-semibold text-ink">{health.mappings}</div>
          </Card>
          <Card title="Open">
            <div className="font-display text-2xl font-semibold text-ink">{health.openMappings}</div>
          </Card>
        </div>
      )}
      ...
      {loading && <p className="mb-4 text-sm text-muted">Loading mappings…</p>}

      <div className="grid grid-cols-1 gap-5 xl:grid-cols-5">
        <div className="space-y-5 xl:col-span-2">
          <Card title="Register mapping">
            <form
              className="space-y-3"
              onSubmit={(event) => {
                event.preventDefault();
                void run(async () => {
                  const input: Parameters<typeof createMapping>[1] = { title };
                  ...
                  const created = await createMapping(token!, input);
                  selectedIdRef.current = created.mapping.id;
                  setSelectedId(created.mapping.id);
                  setTitle("");
                  setDescription("");
                  setOwnerLabel("");
                  setObligationId("");
                  setControlId("");
                  setMessage("Mapping registered");
                });
              }}
            >
              <input
          ...
          >
            {visible.length === 0 && !loading ? (
              <p className="text-sm text-muted">No mappings match the current filter.</p>
            ) : (
        ...
        <div className="xl:col-span-3">
          {!selected ? (
            <Card title="Mapping detail">
              <p className="text-sm text-muted">Select a mapping to activate or retire it.</p>
            </Card>
          ) : (
            <Card title={selected.mappingCode} headerExtra={statusBadge(selected.status)}>
              ...
              {selected.status === "draft" && (
                <Btn
                  ...
                  onClick={() =>
                    void run(async () => {
                      await transitionMapping(token!, selected.id, "activate");
                      setMessage("Mapping activated");
                    })
                  }
                >
                  Activate
              ...
              {selected.status === "active" && (
                <Btn
                  ...
                  onClick={() =>
                    void run(async () => {
                      await transitionMapping(token!, selected.id, "retire");
                      setMessage("Mapping retired");
                    })
                  }
                >
                  Retire
