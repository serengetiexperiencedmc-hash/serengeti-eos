...
import {
  ...
  type ImportBatch,
  type SupplierImportEntityType,
  validateImportBatch,
} from "@/lib/suppliers-api";
import { Btn } from "./ui";

type Step = "configure" | "validate" | "commit" | "done";
...
export function SupplierImportModal({
  ...
  const [entityType, setEntityType] = useState<SupplierImportEntityType>("supplier");
  ...
  async function handleExecute() {
    ...
    setError(null);
    try {
      const key = `ui-import-${batch.id}-${Date.now()}`;
      const result = await executeImportBatch(token, batch.id, key);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink/40 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[10px] border border-line bg-paper shadow-xl">
        <div className="flex items-center justify-between border-b border-line px-5 py-4">
          <div>
            <div className="eyebrow mb-1">Bulk Import</div>
            <h2 className="font-display text-xl font-semibold text-ink">Import Supplier CSV</h2>
          </div>
          ...
          {step === "configure" && (
            <>
              ...
              <label className="block text-sm">
                ...
                <select
                  value={entityType}
                  onChange={(e) => setEntityType(e.target.value as SupplierImportEntityType)}
                  className="w-full rounded-md border border-line bg-ivory px-3 py-2 text-ink"
                >
                  {IMPORT_ENTITY_OPTIONS.map((opt) => (
              ...
              <label className="block text-sm">
                ...
                <textarea
                  ...
                  rows={8}
                  placeholder="supplierCode,legalName,category,country,status..."
          ...
          {step === "validate" && batch && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3 text-center text-sm">
                ...
                <div className="rounded-md border border-line bg-danger-bg p-3">
              ...
              {batch.status === "failed" && (
                <p className="text-sm text-danger">Fix invalid rows and start a new import batch.</p>
              )}
            </div>
          )}
