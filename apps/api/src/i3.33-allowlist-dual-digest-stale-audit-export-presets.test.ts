...
describe("I3.33 named tenant stale allowlist audit export presets", () => {
  it("requires write to save, rejects empty names, and upserts by name", async () => {
    const store = seedStore("i333-presets", TEST_BOOTSTRAP_SECRETS);
    ...
    expect(created.json().increment).toBe("I3.35");
  ...
  it("applies a named preset on stale export and still persists lastFilter", async () => {
    const store = seedStore("i333-export", TEST_BOOTSTRAP_SECRETS);
    const app = buildServer({ store });
    const token = await loginCarol(app);
    ...
    const missing = await app.inject({
      method: "GET",
      url: "/v1/notifications/email/allowlist-dual-digest-stale/export?preset=Unknown",
      headers: { authorization: `Bearer ${token}` },
    });
    ...
    expect(exported.json().increment).toBe("I3.35");
