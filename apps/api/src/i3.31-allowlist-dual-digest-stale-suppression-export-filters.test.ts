...
describe("I3.31 stale allowlist dual digest suppression export filters", () => {
  it("rejects unknown action and inverted window", async () => {
    const store = seedStore("i331-format", TEST_BOOTSTRAP_SECRETS);
    ...
    const badWindow = await app.inject({
      method: "GET",
      url: "/v1/notifications/email/allowlist-dual-digest-stale/export?since=2026-08-24T00:00:00.000Z&until=2026-08-23T00:00:00.000Z",
      headers: { authorization: `Bearer ${token}` },
    });
  ...
  it("filters tenant audit by action and createdAt window", async () => {
    const store = seedStore("i331-filter", TEST_BOOTSTRAP_SECRETS);
    ...
    const snoozeOnly = await app.inject({
      method: "GET",
      url: "/v1/notifications/email/allowlist-dual-digest-stale/export?action=snooze",
      headers: { authorization: `Bearer ${token}` },
    });
    expect(snoozeOnly.statusCode).toBe(200);
    expect(snoozeOnly.json().increment).toBe("I3.31");
    ...
    const last = store.notifAllowlistDualDigestStaleSuppressionAudits
      .filter((row) => row.tenantId !== "22222222-2222-4222-8222-222222222222")
      .at(-1)!;
    const windowed = await app.inject({
      method: "GET",
      url: `/v1/notifications/email/allowlist-dual-digest-stale/export?since=${encodeURIComponent(last.createdAt)}`,
      headers: { authorization: `Bearer ${token}` },
    });
