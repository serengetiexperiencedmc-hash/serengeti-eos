...
/** I4.31 — upsert named tenant DLQ stale-audit export preset. */
export async function upsertNotifDlqSlaDigestStaleAuditExportPreset(
  pool: DbPool,
  row: import("@sedmc/kernel").NotifDlqSlaDigestStaleAuditExportPreset,
): Promise<void> {
  await pool.query(
    `INSERT INTO notif_dlq_sla_digest_stale_audit_export_preset (
...
export async function loadNotifDlqSlaDigestStaleAuditExportPresets(
  pool: DbPool,
): Promise<import("@sedmc/kernel").NotifDlqSlaDigestStaleAuditExportPreset[]> {
  const result = await pool.query(
    `SELECT id, tenant_id, name, action, since_text, until_text,
            created_at, created_by_principal_id, updated_at
     FROM notif_dlq_sla_digest_stale_audit_export_preset`,
  );
  return result.rows.map((row) => {
...
/** I3.32 — upsert last-used allowlist stale-audit export filter. */
export async function upsertNotifAllowlistDualDigestStaleAuditExportLastFilter(
  pool: DbPool,
  row: import("@sedmc/kernel").NotifAllowlistDualDigestStaleAuditExportLastFilter,
): Promise<void> {
  await pool.query(
    `INSERT INTO notif_allowlist_dual_digest_stale_audit_export_last_filter (
      tenant_id, principal_id, action, since_text, until_text, updated_at
    ) VALUES ($1,$2,$3,$4,$5,$6)
     ON CONFLICT (tenant_id, principal_id) DO UPDATE SET
       action = EXCLUDED.action,
...
export async function loadNotifAllowlistDualDigestStaleAuditExportLastFilters(
  pool: DbPool,
): Promise<import("@sedmc/kernel").NotifAllowlistDualDigestStaleAuditExportLastFilter[]> {
  const result = await pool.query(
    `SELECT tenant_id, principal_id, action, since_text, until_text, updated_at
     FROM notif_allowlist_dual_digest_stale_audit_export_last_filter`,
  );
  return result.rows.map((row) => {
    const updatedAt = row.updated_at instanceof Date ? row.updated_at.toISOString() : String(row.updated_at);
    return {
