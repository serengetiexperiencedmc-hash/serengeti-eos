...
import {
  hydrateNotifDlqSlaDigestStaleAuditExportLastPresets,
  hydrateNotifDlqSlaDigestStaleAuditExportPresetUsages,
...
function isI434UsageSql(sql: string) {
  return (
...
function isI429LastFilterSql(sql: string) {
  return sql.includes("notif_dlq_sla_digest_stale_audit_export_last_filter");
}
...
describe("I4.34 persist DLQ preset usage and last-used preset", () => {
  it("lists I4.34 migration without I20 or I3 usage tables", () => {
    const files = listMigrationFiles();
    expect(files.some((f) => f.includes("076_i434_dlq_sla_digest_stale_audit_export_preset_usage"))).toBe(true);
    expect(files.some((f) => f.includes("076_") && f.includes("i2023"))).toBe(false);
    expect(files.some((f) => f.includes("076_") && f.includes("i336"))).toBe(false);
  });

  it("dual-writes usage and last preset on named export and hydrates", async () => {
    const store = seedStore("i434-persist", TEST_BOOTSTRAP_SECRETS);
    const writes: string[] = [];
    const usageWrites: NotifDlqSlaDigestStaleAuditExportPresetUsage[] = [];
    const lastWrites: NotifDlqSlaDigestStaleAuditExportLastPreset[] = [];
    store.dbPool = {
      query: async (sql: string, params?: unknown[]) => {
        const text = String(sql);
        writes.push(text);
        if (text.includes("INSERT INTO notif_dlq_sla_digest_stale_audit_export_preset_usage")) {
          usageWrites.push({
        ...
        if (text.includes("INSERT INTO notif_dlq_sla_digest_stale_audit_export_last_preset")) {
          lastWrites.push({
        ...
        if (text.includes("FROM notif_dlq_sla_digest_stale_audit_export_preset_usage") && !text.includes("INSERT")) {
          return {
        ...
        if (text.includes("FROM notif_dlq_sla_digest_stale_audit_export_last_preset") && !text.includes("INSERT")) {
          return {
    ...
    expect(writes.filter(isI434UsageSql)).toHaveLength(0);
    ...
    expect(usageWrites).toHaveLength(0);
    expect(lastWrites).toHaveLength(0);
    expect(writes.some(isI429LastFilterSql)).toBe(true);
    ...
    const invalid = await app.inject({
      method: "GET",
      url: "/v1/notifications/email/dlq-sla-digest-stale/export?since=not-a-date",
      headers: { authorization: `Bearer ${token}` },
    });
    ...
    const persistAfterApply = writes.filter(isI434UsageSql).length;
    ...
    expect(writes.filter(isI434UsageSql)).toHaveLength(persistAfterApply);
    ...
    const persistAfterRepeat = writes.filter(isI434UsageSql).length;
    ...
    expect(writes.filter(isI434UsageSql)).toHaveLength(persistAfterRepeat);
    ...
    const renamed = await app.inject({
      method: "POST",
      url: `/v1/notifications/email/dlq-sla-digest-stale/export/presets/${presetId}/rename`,
      headers: { authorization: `Bearer ${token}` },
    ...
    const emptyStore = seedStore("i434-hydrate", TEST_BOOTSTRAP_SECRETS);
    const mergedUsages = await hydrateNotifDlqSlaDigestStaleAuditExportPresetUsages(store.dbPool!, emptyStore);
    const mergedLast = await hydrateNotifDlqSlaDigestStaleAuditExportLastPresets(store.dbPool!, emptyStore);
    expect(mergedUsages).toBe(2);
    expect(mergedLast).toBe(1);
    expect(emptyStore.notifDlqSlaDigestStaleAuditExportPresetUsages).toHaveLength(2);
    expect(emptyStore.notifDlqSlaDigestStaleAuditExportLastPresets[0]!.presetName).toBe("Snoozes only");
    expect(emptyStore.notifDlqSlaDigestStaleAuditExportPresetUsages.every((row) => row.presetName === snapshotName)).toBe(
      true,
    );
    await persistNotifDlqSlaDigestStaleAuditExportPresetUsage(store.dbPool, usageWrites[0]!);
    await persistNotifDlqSlaDigestStaleAuditExportLastPreset(store.dbPool, lastWrites[0]!);
    expect(typeof insertNotifDlqSlaDigestStaleAuditExportPresetUsage).toBe("function");
    expect(typeof upsertNotifDlqSlaDigestStaleAuditExportLastPreset).toBe("function");
    expect(typeof loadNotifDlqSlaDigestStaleAuditExportPresetUsages).toBe("function");
    expect(typeof loadNotifDlqSlaDigestStaleAuditExportLastPresets).toBe("function");
  });

  it("swallows persist throw and stays memory-only without dbPool", async () => {
    const throwing = seedStore("i434-throw", TEST_BOOTSTRAP_SECRETS);
    throwing.dbPool = {
      query: async (sql: string) => {
        if (String(sql).includes("notif_dlq_sla_digest_stale_audit_export_preset_usage")) {
    ...
    const thrown = await throwApp.inject({
      method: "GET",
      url: "/v1/notifications/email/dlq-sla-digest-stale/export?preset=Throw%20persist",
      headers: { authorization: `Bearer ${throwToken}` },
    });
    expect(thrown.statusCode).toBe(200);
    expect(throwing.notifDlqSlaDigestStaleAuditExportPresetUsages).toHaveLength(1);

    const memory = seedStore("i434-memory", TEST_BOOTSTRAP_SECRETS);
    ...
    const memoryExport = await memoryApp.inject({
      method: "GET",
      url: "/v1/notifications/email/dlq-sla-digest-stale/export?preset=Memory%20only",
      headers: { authorization: `Bearer ${memoryToken}` },
    });
    expect(memoryExport.statusCode).toBe(200);
    expect(memoryExport.json().lastPreset.presetName).toBe("Memory only");
    expect(memory.notifDlqSlaDigestStaleAuditExportPresetUsages).toHaveLength(1);
