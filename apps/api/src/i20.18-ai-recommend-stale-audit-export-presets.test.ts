...
describe("I20.18 named tenant stale recommend audit export presets", () => {
  it("requires write to save, rejects empty names, and upserts by name", async () => {
    const store = seedStore("i2018-presets", TEST_BOOTSTRAP_SECRETS);
    ...
    expect(empty.json().increment).toBe("I20.18");
  ...
  it("applies a named preset on stale export without recording a recommend run", async () => {
    const store = seedStore("i2018-export", TEST_BOOTSTRAP_SECRETS);
    const app = buildServer({ store });
    const token = await loginCarol(app);
    ...
    const missing = await app.inject({
      method: "GET",
      url: "/v1/ai/recommendations/last-run/stale/export?preset=Unknown",
      headers: { authorization: `Bearer ${token}` },
    });
    ...
    expect(exported.json().increment).toBe("I20.18");
