...
export function addDlqSlaDigestRecipient(
  ...
  if (existing) {
    ...
    return { recipient: sanitize(existing), updated: true, increment: "I4.17" as const };
  }
  ...
  return { recipient: sanitize(recipient), updated: false, increment: "I4.17" as const };
}
...
export function revokeDlqSlaDigestRecipient(store: Store, principal: Principal, id: string) {
  ...
  return { recipient: sanitize(entry), increment: "I4.17" as const };
}
