...
describe("E2 ERM Treatment register", () => {
  it("lists E2 migration", () => {
    expect(listMigrationFiles().some((f) => f.includes("108_e2_erm_treatments"))).toBe(true);
  });
  ...
  it("enforces auth and tenant isolation without broadening risk.member", async () => {
    ...
    expect((await app.inject({ method: "GET", url: "/v1/erm/treatments/health" })).statusCode).toBe(401);
    expect(
      (
        await app.inject({
          method: "GET",
    ...
    expect(health.json().increment).toBe("E2");
    expect(health.json().module).toBe("erm-treatments");
    expect(health.json().treatments).toBe(0);
    expect(health.json().openTreatments).toBe(0);
    ...
    const missing = await app.inject({
      method: "GET",
      url: `/v1/erm/treatments/${newId()}`,
      headers: { authorization: `Bearer ${carolToken}` },
    });
    expect(missing.statusCode).toBe(404);

    expect(store.roles.find((r) => r.key === "erm.treatment")?.permissionKeys).toEqual([
      "erm:read:treatment",
      "erm:write:treatment",
    ]);
    ...
    expect(store.roles.find((r) => r.key === "risk.member")?.permissionKeys).not.toContain(
      "erm:read:treatment",
    );
    expect(store.roles.find((r) => r.key === "risk.member")?.permissionKeys).not.toContain(
      "erm:write:treatment",
    );
    expect(store.roles.find((r) => r.key === "erm.kri")?.permissionKeys).not.toContain(
      "erm:read:treatment",
    );
  ...
  it("runs Treatment lifecycle with human-only mutate, optional riskId, and no I15 mutation", async () => {
    const store = seedStore("test-secret");
    const app = buildServer({ store });
    const carolToken = await login(app, "carol.admin@sedmc.local", P.carolPassword, "sedmc");
    ...
    const foreignTreatmentId = "97979797-9797-4979-8979-979797979797";
    const foreignRiskId = "98989898-9898-4989-8989-989898989898";
    store.ermTreatments.push({
      id: foreignTreatmentId,
      tenantId: partnerTenant!.id,
      treatmentCode: "TRT-9999",
      title: "Other tenant treatment",
      status: "open",
    ...
    const crossTenant = await app.inject({
      method: "GET",
      url: `/v1/erm/treatments/${foreignTreatmentId}`,
      headers: { authorization: `Bearer ${carolToken}` },
    });
    ...
    const created = await app.inject({
      ...
      headers: { authorization: `Bearer ${carolToken}` },
      payload: {
        title: "Supplier continuity treatment",
        notes: "Catalogue row only — not a treatment engine",
        ownerLabel: "CRO (Dev/Test)",
        status: "retired",
      },
    });
    expect(created.statusCode).toBe(201);
    expect(created.json().treatment.treatmentCode).toBe("TRT-0001");
    expect(created.json().treatment.status).toBe("open");
    expect(created.json().treatment.title).toBe("Supplier continuity treatment");
    expect(created.json().treatment.riskId).toBeUndefined();
    expect(created.json().treatment.effectiveness).toBeUndefined();
    expect(created.json().treatment.residualRisk).toBeUndefined();
    expect(created.json().treatment.kriId).toBeUndefined();
    expect(created.json().treatment.dueDate).toBeUndefined();
    expect(created.json().treatment.cost).toBeUndefined();
    expect(created.json().treatment.priority).toBeUndefined();
    expect(created.json().treatment.ragStatus).toBeUndefined();
    assertNoSecrets(created.json());
    const id = created.json().treatment.id as string;
    ...
    expect(linked.json().treatment.treatmentCode).toBe("TRT-0002");
    expect(linked.json().treatment.riskId).toBe(ERM_SEED.sampleRiskId);
    expect(linked.json().treatment.riskCode).toBe("RSK-0001");
    const linkedId = linked.json().treatment.id as string;
    ...
    expect(listed.json().items[0].treatmentCode).toBe("TRT-0002");
    ...
    expect(got.json().treatment.treatmentCode).toBe("TRT-0001");

    const patched = await app.inject({
      method: "PATCH",
      url: `/v1/erm/treatments/${id}`,
      headers: { authorization: `Bearer ${carolToken}` },
      payload: { title: "Supplier continuity treatment — register only" },
    });
    expect(patched.json().treatment.title).toBe("Supplier continuity treatment — register only");

    const cleared = await app.inject({
      method: "PATCH",
      url: `/v1/erm/treatments/${linkedId}`,
      headers: { authorization: `Bearer ${carolToken}` },
      payload: { riskId: null },
    });
    expect(cleared.statusCode).toBe(200);
    expect(cleared.json().treatment.riskId).toBeUndefined();
    expect(cleared.json().treatment.riskCode).toBeUndefined();
    ...
    expect(reLinked.json().treatment.riskId).toBe(ERM_SEED.sampleRiskId);
    ...
    expect(retired.json().treatment.status).toBe("retired");
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/v1/erm/risks/${ERM_SEED.sampleRiskId}/treatment`,
          headers: { authorization: `Bearer ${carolToken}` },
        })
      ).statusCode,
    ).toBe(404);
    expect(
      (
        await app.inject({
          method: "GET",
          url: `/v1/erm/risks/${ERM_SEED.sampleRiskId}/treatments`,
          headers: { authorization: `Bearer ${carolToken}` },
        })
      ).statusCode,
    ).toBe(404);
    ...
    expect("ermTreatment" in store).toBe(false);
